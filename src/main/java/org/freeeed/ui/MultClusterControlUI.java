...
import org.freeeed.ec2.MultCluster;
import org.freeeed.ec2.Server;
 ...
 * @author mark
 */
public class MultClusterControlUI extends javax.swing.JDialog {
     ...
     */
    public MultClusterControlUI(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private MultCluster multiCluster = new MultCluster();
    ...
    public class StatusRefresher implements Runnable {
        ...
        public void run() {
            ...
            try {
                multiCluster.refreshClusters();
                
                StringBuffer result = new StringBuffer();
                
                List<Cluster> clusters = multiCluster.getClusters();
                int count = 1;
                for (Cluster cluster : clusters) {
                    ...
                    for (Server server : cluster) {
                        result.append(server.getInstanceId()).append(" ").
                                ...
                                append(server.getInitialized()).append(" Mode: ");
                        result.append(server.isNameNode() ? "NN " : "");
                        result.append(server.isJobTracker() ? "JT " : "");
                        result.append(server.isSecondaryNameNode() ? "SNN " : "");
                        result.append(server.isDataNode() ? "DN " : "");
                        result.append(server.isTaskTracker() ? "TT " : "");
                        result.append("    ").append(server.getDnsName());
                        result.append("\n");
                    }
                    
                    if (cluster.getRunningCount() < Settings.getSettings().getClusterSize()
                            || cluster.getInitializedCount() < Settings.getSettings().getClusterSize()) {
                        
                        result.append("Waiting for all " + Settings.getSettings().getClusterSize() + " instances to come up\n");
                    }
                    
                    result.append("Cluster ready: " + cluster.isReadyToUse());
    ...
    private void startCluster() {
        int clusters = 1;
        if (Settings.getSettings().isTrainingClusters()) {
            clusters = Settings.getSettings().getNumberTrainingClusters();
        }
        
        History.appendToHistory("Starting " + clusters + " clusters of " + Settings.getSettings().getClusterSize() + " nodes.");
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    int numberOfInstancesNeeded = Settings.getSettings().getClusterSize();
                    if (Settings.getSettings().isTrainingClusters()) {
                        numberOfInstancesNeeded *= Settings.getSettings().getNumberTrainingClusters();
                    }

                    agent.setClusterSize(numberOfInstancesNeeded);
                    ...
                    int attempt = 0;
                    while (!allInstancesUp(numberOfInstancesNeeded)) {
                        ++attempt;
                ...
                for (Cluster cluster : clusters) {
                    ...
                    //hadoopAgent.setCallingUI(MultClusterControlUI.this);
                    hadoopAgent.setupAndStartMC(cluster);
                    refreshStatus();
                }
            ...
            public void run() {
                try {
                    ...
                    for (Cluster cluster : clusters) {
                        ...
                        hadoopAgent.checkHealthMC(cluster);
                        ready = ready && HadoopAgent.isHadoopReady();
                    }

                    HadoopAgent.setHadoopReady(ready);
                    refreshStatus();
    ...
    private boolean allInstancesUp(int numberOfInstancesNeeded) {
        EC2Agent agent = new EC2Agent();
        Cluster cluster = agent.getRunningInstances(true);
        return cluster.size() == numberOfInstancesNeeded
                && cluster.getInitializedCount() == numberOfInstancesNeeded;
    }
